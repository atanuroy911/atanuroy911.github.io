		...
		jQuery(document).ready(function() {		
			...
			jQuery("a[href$='.pdf']").each(function() {
				if(gethost.test(jQuery(this).attr('href'))){
					...
					jQuery(this).attr("href", '<?php echo esc_url( $viewer_url ); ?>' + _href + '#zoom=<?php echo esc_html( $tnc_pvfw_default_zoom ); ?>');
				} else {
					// Do Nothing
				}
			});
	...
	<script type="text/javascript">
		jQuery(document).ready(function() {		
			...
			jQuery("a[href$='.pdf']").each(function() {
				if(gethost.test(jQuery(this).attr('href'))){
					var _href = jQuery(this).attr("href");
					jQuery(this).replaceWith("<a class='fullscreen-mode' href='<?php echo esc_url( $viewer_url ); ?>" + _href +"#zoom=<?php echo esc_html( $tnc_pvfw_default_zoom ); ?>' ><?php echo esc_html( $fullscreen_text ); ?></a><br><iframe width='<?php echo esc_attr( $auto_iframe_width ); ?>' height='<?php echo esc_attr( $auto_iframe_height ); ?>' src='<?php echo esc_url( $viewer_url ); ?>" + _href +"#zoom=<?php echo esc_html( $tnc_pvfw_default_zoom ); ?>'></iframe>");
				} else {
					// do nothing.
				}
			});
