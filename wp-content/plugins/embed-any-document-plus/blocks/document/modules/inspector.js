...
class EadInspector extends Component {
    ...
    render() {
		...
		if ( enableViewerControl ) {
			viewerOptions = [{ value: 'google', label: __( 'Google Docs Viewer', 'embed-any-document-plus' ) }];
			...
			if ( fileSrc !== 'dropbox' ) {
				if ( EadHelper.isPDF(url) ) {
					viewerOptions.push({ value: 'browser', label: __( 'Browser Based', 'embed-any-document-plus' ) });

					if (jQuery.inArray( 'built-in', emebeder.viewers ) !== -1 && fileSrc === 'internal') {
						viewerOptions.push({ value: 'built-in', label: __( 'Built-In Viewer', 'embed-any-document-plus' ) });
					}
				}
			} else {
				viewerOptions.push({ value: 'dropbox', label: __( 'Dropbox', 'embed-any-document-plus' ) });
			}

			downloadTextControl = <TextControl label={ __( 'Download Text', 'embed-any-document-plus' ) } help={ __( 'Default download button text', 'embed-any-document-plus' ) } value={ text } onChange={ text => setAttributes( { text } ) } />;
