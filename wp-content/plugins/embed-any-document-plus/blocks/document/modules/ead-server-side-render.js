			...
			if ( ( attributes !== null && attributes ) && ( attributes.viewer === 'google' || attributes.viewer === 'browser' || attributes.viewer === 'built-in' || attributes.viewer === 'dropbox' ) ) {
				...
				if ( viewer !== 'dropbox' ) {
					...
					if ( viewer === 'browser' || viewer === 'built-in' ) {
						...
						let isBuiltInViewer = eadPublic.pdfjs && eadPublic.pdfjs.length > 0 && viewer === 'built-in';
...
EadServerSideRender.defaultProps = {
	EmptyResponsePlaceholder: ( { className } ) => (
		<Placeholder label={ __( 'Document', 'embed-any-document-plus' ) } icon={ icon.block } className={ className }>
			{ __( 'No document found!', 'embed-any-document-plus' ) }
		</Placeholder>
	),
	ErrorResponsePlaceholder: ( { response, className } ) => {
		const errorMessage = sprintf(
			// translators: %s: error message describing the problem
			__( 'Error loading the document: %s', 'embed-any-document-plus' ),
			response.errorMsg
		);
		return (
			<Placeholder label={ __( 'Document', 'embed-any-document-plus' ) } icon={ icon.block } className={ className }>{ errorMessage }</Placeholder>
		);
	},
